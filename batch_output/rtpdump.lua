...
-- field declarations
f.file_header = ProtoField.bytes("rtpdump.file_header", "file_header")
f.packets = ProtoField.bytes("rtpdump.packets", "packets")
f.shebang = ProtoField.bytes("rtpdump.header_t.shebang", "shebang")
f.space = ProtoField.bytes("rtpdump.header_t.space", "space")
f.ip = ProtoField.bytes("rtpdump.header_t.ip", "ip")
f.port = ProtoField.bytes("rtpdump.header_t.port", "port")
f.start_sec = ProtoField.bytes("rtpdump.header_t.start_sec", "start_sec")
f.start_usec = ProtoField.bytes("rtpdump.header_t.start_usec", "start_usec")
f.ip2 = ProtoField.bytes("rtpdump.header_t.ip2", "ip2")
f.port2 = ProtoField.bytes("rtpdump.header_t.port2", "port2")
f.padding = ProtoField.bytes("rtpdump.header_t.padding", "padding")
f.length = ProtoField.bytes("rtpdump.packet_t.length", "length")
f.len_body = ProtoField.bytes("rtpdump.packet_t.len_body", "len_body")
f.packet_usec = ProtoField.bytes("rtpdump.packet_t.packet_usec", "packet_usec")
f.body = ProtoField.bytes("rtpdump.packet_t.body", "body")
